...
func main() {
	...
	// --init-config is a one-shot writer — never falls through to TUI startup
	// or the API-key gate, so a brand-new user can `bitchtea --init-config`
	// before they have an API key set.
	if opts.createConfigDefaults {
		if err := runCreateConfigDefaults(&cfg, opts.forceOverwrite); err != nil {
			fmt.Fprintf(os.Stderr, "bitchtea: %v\n", err)
			os.Exit(1)
		}
		return
...
func parseCLIArgs(args []string, cfg *config.Config) (cliOptions, error) {
	...
	for i := 0; i < len(args); i++ {
		...
		case "--bare":
			opts.bare = true
			cfg.Bare = true
		case "--init-config", "--create-config-defaults", "--write-defaults":
			// --create-config-defaults is the original name; --init-config and
			// --write-defaults are the names users actually reach for. All three
			// route to the same writer.
...
// setKeyDocs is a one-line description per recognised /set key. Used by
// --init-config to render a self-documenting bitchtearc so users can edit
// instead of guess. Keep entries short — one screen-line each, no wrap.
//
// If a key in config.SetKeys() is missing here, runCreateConfigDefaults
// falls back to "(no description)" rather than erroring. Adding the doc
// here is a soft contract, not a build break.
var setKeyDocs = map[string]string{
	"provider":           "wire format / API dialect: openai or anthropic",
	"model":              "model identifier the active provider speaks (e.g. gpt-4o, claude-opus-4-7)",
	"apikey":             "API key for the active provider; overrides env vars",
	"baseurl":            "base URL for OpenAI-compatible endpoints (Ollama, OpenRouter, Z.ai, ...)",
	"nick":               "your IRC-style nickname shown in chat",
	"profile":            "named profile to apply at startup (ollama, openrouter, zai-openai, zai-anthropic, or your saved profile)",
	"service":            "upstream service identity, distinct from wire provider; gates per-service behavior",
	"sound":              "play notification sound on agent finish: on or off",
	"auto-next":          "auto-inject next-step prompts after each turn: on or off",
	"auto-idea":          "auto-generate improvement ideas after each turn: on or off",
	"persona_file":       "path to a persona prompt file loaded on top of the system prompt",
	"top_k":              "sampling top-k integer; <unset> defers to the provider default",
	"top_p":              "sampling top-p float in [0,1]; <unset> defers to the provider default",
	"temperature":        "sampling temperature float; <unset> defers to the provider default",
	"repetition_penalty": "repetition penalty float; <unset> defers to the provider default",
	"tool_verbosity":     "tool output detail: terse, normal, or verbose",
	"banner":             "print the splash banner at startup: on or off",
	"effort":             "reasoning effort hint: low, medium, high, max (or empty for provider default)",
	"tool_timeout":       "per-tool timeout in seconds (positive integer)",
}

// runCreateConfigDefaults writes a self-documenting ~/.bitchtea/bitchtearc:
// one block per recognised SET key consisting of a "# <doc>" comment and a
// commented-out "# set <key> <current_default>" line. Users uncomment and
// edit the lines they care about; the rest stays as inline reference.
func runCreateConfigDefaults(cfg *config.Config, force bool) error {
	rcPath := config.RCPath()
	if _, err := os.Stat(rcPath); err == nil && !force {
		fmt.Printf("~/.bitchtea/bitchtearc already exists. Re-run with --force to overwrite, or pass: bitchtea --init-config --force\n")
		return nil
	}

	keys := config.SetKeys()
	var sb strings.Builder
	sb.WriteString("# bitchtea runtime config — generated by --init-config\n")
	sb.WriteString("# Each block is one /set key: a one-line doc, then a commented\n")
	sb.WriteString("# `set <key> <current_default>` line. Uncomment to activate.\n")
	sb.WriteString("#\n")
	for _, key := range keys {
		val, _ := config.GetSetting(cfg, key)
		doc, ok := setKeyDocs[key]
		if !ok {
			doc = "(no description)"
		}
	...
	fmt.Printf("Wrote %s (%d keys, each documented and commented out)\n", rcPath, len(keys))
	fmt.Println("Uncomment any `# set ...` line to activate it.")
	return nil
}
...
Flags:
  ...
  --init-config          Write a documented bitchtearc to ~/.bitchtea/bitchtearc and exit
                         (aliases: --create-config-defaults, --write-defaults)
  --force                With --init-config, overwrite an existing rc file without prompting
