...
| `/set <key> [value]` | Show or change settings (`sound`, `auto-next`, `auto-idea`, `provider`, `model`, `baseurl`, `apikey`, `service`, `nick`, `profile`). Off by default; turn on with `/set auto-next on`. `provider` is the wire format (`openai` or `anthropic`); `service` is the upstream identity used for per-service gates — see [docs/commands.md](docs/commands.md) and [docs/archive/phase-9-service-identity.md](docs/archive/phase-9-service-identity.md). | Moderate when toggling auto-next/auto-idea (changes turn behavior) |
